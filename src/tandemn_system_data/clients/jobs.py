...
Status semantics for the scheduler:
  ...
  running        status in {launching, running}   (launching jobs
                 already hold resources, so Koi must see them)
  active chains  chain.status in {launching, running, draining}
                 (draining chains still occupy GPUs)
...
from tandemn_system_data.db.orm import ChainRow, JobRow, PlanJobRow, RankRow
...
WAITING_STATUSES: tuple[JobStatus, ...] = (JobStatus.SUBMITTED,)
RUNNING_STATUSES: tuple[JobStatus, ...] = (JobStatus.LAUNCHING, JobStatus.RUNNING)
...
class JobStore:
    ...
    def waiting_jobs(self, user_id: str) -> list[Job]:
        with self._client.session() as s:
            rows = s.scalars(
                select(JobRow)
                .where(JobRow.user_id == user_id, JobRow.status.in_(WAITING_STATUSES))
                .order_by(JobRow.created_at)
            ).all()
            return [Job.model_validate(r) for r in rows]

    def running_jobs(self, user_id: str) -> list[RunningJob]:
        """Launching/running jobs plus the active chains serving them,
        joined through plan_jobs -> ranks -> chains."""
        with self._client.session() as s:
            job_rows = s.scalars(
                select(JobRow)
                .where(JobRow.user_id == user_id, JobRow.status.in_(RUNNING_STATUSES))
                .order_by(JobRow.created_at)
            ).all()
            if not job_rows:
                return []
            ...
            chain_rows = s.execute(
                select(PlanJobRow.job_id, PlanJobRow.plan_id, ChainRow)
                .join(RankRow, RankRow.plan_id == PlanJobRow.plan_id)
                .join(ChainRow, ChainRow.rank_id == RankRow.rank_id)
                .where(
                    PlanJobRow.job_id.in_(job_ids),
            ...
            for job_id, plan_id, chain in chain_rows:
                chains_by_job.setdefault(job_id, []).append(
                    ChainAllocation(
                        chain_id=chain.chain_id,
                        rank_id=chain.rank_id,
                        plan_id=plan_id,
                        role=chain.role,
                        status=chain.status,
                        shape_json=chain.shape_json,
                        parallelism_json=chain.parallelism_json,
