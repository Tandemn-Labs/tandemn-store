...
**Schema invariants:** spine is `users`, `jobs`, `plans`, `ranks`, `events`(+offsets), `credentials`, `resource_maps`, plus Koi-only `evidence_rows` and `koi_causal_*` (nodes, edges+metadata, mechanisms+metadata). Job statuses: `waiting | running | paused | finished`. Plans are `tick_rationale` + `actions_json`. Ranks are job-scoped and carry their replica count. One `resource_maps` row per user; `pools_json` is hierarchical (`clouds → regions → zones → network_fabrics → machine_pools`). Do not re-add `koi_ticks`, `chains`, `plan_jobs`, `attempts`, or `outcomes`. No throughput columns or rank traversal in the MVP.
