...
@pytest.fixture
def seeded_user(pg_client: PostgresClient) -> str:
    """Insert a user for FK satisfaction; return its id."""
    from tandemn_system_data.db import UserRow

    tid = new_user_id()
    now = datetime.now(UTC)
    with pg_client.begin() as s:
        s.add(UserRow(user_id=tid, name="creds-test", created_at=now))
    return tid
...
def test_put_returns_credentials_ref(store: CredentialStore, seeded_user: str):
    ref = store.put(
        user_id=seeded_user,
...
def test_put_then_get_round_trip(store: CredentialStore, seeded_user: str):
    ref = store.put(
        user_id=seeded_user,
        scope_json={"prefix": "s3://customer/inputs/"},
...
def test_put_accepts_caller_supplied_ref(store: CredentialStore, seeded_user: str):
...
def test_put_rejects_naive_expires_at(store: CredentialStore, seeded_user: str):
...
def test_put_rejects_past_expires_at(store: CredentialStore, seeded_user: str):
    with pytest.raises(ValueError):
        store.put(
            user_id=seeded_user,
            scope_json={},
...
def test_put_rejects_non_bytes_secret(store: CredentialStore, seeded_user: str):
...
def test_put_rejects_non_json_secret(store: CredentialStore, seeded_user: str):
...
def test_get_rejects_expired(store: CredentialStore, seeded_user: str, pg_client: PostgresClient):
...
def test_exists_handles_both_failure_modes(
    store: CredentialStore, seeded_user: str, pg_client: PostgresClient
):
...
def test_list_for_user_excludes_expired_by_default(
    store: CredentialStore, seeded_user: str, pg_client: PostgresClient
):
    live_ref = store.put(
        user_id=seeded_user,
...
def test_purge_expired(store: CredentialStore, seeded_user: str, pg_client: PostgresClient):
    ref_a = store.put(
        user_id=seeded_user,
        scope_json={"label": "a"},
        secret_payload=b'"x"',
